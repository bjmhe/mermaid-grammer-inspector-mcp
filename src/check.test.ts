...
describe("checkMermaid", () => {
  ...
  beforeEach(async () => {
    ...
    mockParseMermaid = parseModule.parseMermaid as unknown as ReturnType<typeof vi.fn>;
  });
...
flowchart TD
  ...
  it("should write file using UTF-8 encoding", async () => {
    ...
    expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.any(String), mermaidText, {
      encoding: "utf-8",
    });
  });
});
